...
export default function ContactResponseDialog({ open, setOpen, data, updateContactData }) {
    const handleClose = () => {
        setOpen(false);
    };
    return (
        <Fragment>
            ...
            >
                ...
                <DialogActions>
                    {
                        !data?.viewed && (
                            <Button  onClick={()=>{handleClose(); updateContactData(data._id);}} style={{ backgroundColor: '#0688cf', color: '#fff', fontSize: "12px" }} autoFocus>
                                Action Taken
                            </Button>
                        )
                    }
